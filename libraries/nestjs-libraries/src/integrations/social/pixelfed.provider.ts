...
export class PixelfedProvider extends SocialAbstract implements SocialProvider {
  override maxConcurrentJob = 5; // Pixelfed instances typically have generous limits
  identifier = 'pixelfed';
  name = 'Pixelfed';
  ...
  protected generateUrlDynamic(
    ...
    return `${customUrl}/oauth/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(
      `${url}/integrations/social/pixelfed`
  ...
  async generateAuthUrl() {
    ...
    const url = this.generateUrlDynamic(
      process.env.PIXELFED_URL || 'https://pixelfed.social',
      state,
      process.env.PIXELFED_CLIENT_ID!,
  ...
  protected async dynamicAuthenticate(
    ...
    form.append(
      'redirect_uri',
      `${process.env.FRONTEND_URL}/integrations/social/pixelfed`
    );
  ...
  async authenticate(params: {
    ...
    return this.dynamicAuthenticate(
      process.env.PIXELFED_CLIENT_ID!,
      process.env.PIXELFED_CLIENT_SECRET!,
      process.env.PIXELFED_URL || 'https://pixelfed.social',
      params.code
    );
  }
